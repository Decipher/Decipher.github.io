...
function server_pid_file(): string {
  ...
  return sprintf('/tmp/quickstart-serverless-drupal-php-server-%s.pid', hash('sha256', (string) getcwd()));
}
...
/**
 * Run a command via passthru, retrying on failure before giving up.
 *
 * For long network-bound steps (composer install): registries and GitHub
 * throw transient 5xx errors, and the step resumes from cache - a retry
 * finishes the job instead of failing the whole first run.
 */
function passthru_with_retries(string $cmd, int $attempts = 3, int $delay_seconds = 10): void {
  for ($attempt = 1; $attempt <= $attempts; $attempt++) {
    ...
    if ($attempt < $attempts) {
      NOTE('Attempt %d of %d failed (exit code %d) - retrying in %d seconds.', $attempt, $attempts, $exit_code, $delay_seconds);
      sleep($delay_seconds);
    }
  }
  FAIL('Command failed after %d attempts: %s', $attempts, $cmd);
}
...
// @codeCoverageIgnoreStart
if (!function_exists('QuickstartServerlessDevTools\quit') && !class_exists('PHPUnit\\Framework\\TestCase')) {
