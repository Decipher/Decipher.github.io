<template>
  <!--
    ...
    It draws its own bar rather than being put in one, because the front page
    has no trail and a bar drawn around nothing is an empty strip.
  -->
  <nav
    v-if="crumbs.length"
    class="border-b border-hairline bg-paper/95 backdrop-blur"
    aria-label="Breadcrumb"
    data-testid="breadcrumb"
  >
    <div
      class="mx-auto flex h-11 w-full max-w-5xl flex-wrap items-center gap-x-2 gap-y-1 px-6 font-mono text-[0.6875rem] uppercase tracking-eyebrow text-dimmed"
    >
      <template v-for="(crumb, index) of crumbs">
        ...
        <!--
          The last crumb is this page. Marked as current for anything reading
          the page aloud, and not a link, because a link to here goes nowhere.
        -->
